...
__device__ glm::vec3 totalMie(glm::vec3 primaryWavelengths, glm::vec3 K,
  ...
  return 0.434f * c * pi *
         glm::pow((2.0f * pi) / primaryWavelengths, glm::vec3(v - 2.0)) * K;
}

__device__ float hgPhase(float cosViewSunAngle, float g) {
  return (1.0 / (4.0 * pi)) *
         ((1.0 - powf(g, 2.0)) /
          pow(1.0 - 2.0 * g * cosViewSunAngle + powf(g, 2.0), 1.5));
}

__device__ float SunIntensity(float zenithAngleCos) {
  return sunIntensity *
         glm::max(0.0, 1.0 - exp(-((cutoffAngle - acos(zenithAngleCos)) /
                                   steepness)));
}
...
__device__ glm::vec3 sun(glm::vec3 viewDir) {
  ...
  float sunE =
      SunIntensity(cosSunUpAngle);  // Get sun intensity based on how high in
  ...
  // mie coefficients
  glm::vec3 mieAtX =
      totalMie(primaryWavelengths, K, turbidity) * mieCoefficient;
  ...
  glm::vec3 Fex =
      exp(-(rayleighAtX * rayleighOpticalLength + mieAtX * mieOpticalLength));
...
__device__ glm::vec3 sunsky(glm::vec3 viewDir) {
  ...
  glm::vec3 sky = somethingElse * (1.0f - Fex);
  sky *=
      glm::mix(glm::vec3(1.0f), glm::pow(somethingElse * Fex, glm::vec3(0.5f)),
               glm::clamp(pow(1.0f - dot(up, sunDirection), 5.0f), 0.0f, 1.0f));
