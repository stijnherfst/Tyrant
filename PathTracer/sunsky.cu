...
__device__ glm::vec3 sun(glm::vec3 viewDir) {
	...
	sky *= mix(glm::vec3(1.0), pow(somethingElse * Fex, glm::vec3(0.5f)), glm::clamp(pow(1.0f - dot(up, sunDirection), 5.0f), 0.0f, 1.0f));
...
__device__ glm::vec3 sky(glm::vec3 viewDir) {
	...
	glm::vec3 sky = somethingElse * (1.0f - Fex);
	sky *= mix(glm::vec3(1.0), glm::pow(somethingElse * Fex, glm::vec3(0.5f)), glm::clamp(pow(1.0f - dot(up, sunDirection), 5.0f), 0.0f, 1.0f));
...
__device__ glm::vec3 sunsky(glm::vec3 viewDir) {
	...
	glm::vec3 sky = somethingElse * (1.0f - Fex);
	sky *= glm::mix(glm::vec3(1.0f), glm::pow(somethingElse * Fex, glm::vec3(0.5f)), glm::clamp(pow(1.0f - dot(up, sunDirection), 5.0f), 0.0f, 1.0f));

	// composition + solar disc
	float sundisk = glm::smoothstep(sunAngularDiameterCos, sunAngularDiameterCos + 0.00002f, cosViewSunAngle);
	glm::vec3 sun = (sunE * 19000.0f * Fex)*sundisk;
