...
CudaMesh load_object(const std::string& source) {
	...
	for (unsigned i = 0; i < cuda_mesh.trianglesNo; i++) {
		minp = glm::min(minp, cuda_mesh.vertices[cuda_mesh.triangles[i].index1].position);
		minp = glm::min(minp, cuda_mesh.vertices[cuda_mesh.triangles[i].index2].position);
		minp = glm::min(minp, cuda_mesh.vertices[cuda_mesh.triangles[i].index3].position);

		maxp = glm::max(maxp, cuda_mesh.vertices[cuda_mesh.triangles[i].index1].position);
		maxp = glm::max(maxp, cuda_mesh.vertices[cuda_mesh.triangles[i].index2].position);
		maxp = glm::max(maxp, cuda_mesh.vertices[cuda_mesh.triangles[i].index3].position);
	}
	...
	for (unsigned i = 0; i < cuda_mesh.trianglesNo; i++) {
		cuda_mesh.triangles[i].bottom = glm::min(cuda_mesh.triangles[i].bottom, cuda_mesh.vertices[cuda_mesh.triangles[i].index1].position);
		cuda_mesh.triangles[i].bottom = glm::min(cuda_mesh.triangles[i].bottom, cuda_mesh.vertices[cuda_mesh.triangles[i].index2].position);
		cuda_mesh.triangles[i].bottom = glm::min(cuda_mesh.triangles[i].bottom, cuda_mesh.vertices[cuda_mesh.triangles[i].index3].position);
		cuda_mesh.triangles[i].top = glm::max(cuda_mesh.triangles[i].top, cuda_mesh.vertices[cuda_mesh.triangles[i].index1].position);
		cuda_mesh.triangles[i].top = glm::max(cuda_mesh.triangles[i].top, cuda_mesh.vertices[cuda_mesh.triangles[i].index2].position);
		cuda_mesh.triangles[i].top = glm::max(cuda_mesh.triangles[i].top, cuda_mesh.vertices[cuda_mesh.triangles[i].index3].position);
	}
	...
	for (unsigned i = 0; i < cuda_mesh.trianglesNo; i++) {
		...
		glm::vec3 vertex1 = cuda_mesh.vertices[triangle.index2].position - cuda_mesh.vertices[triangle.index1].position;
		glm::vec3 vertex2 = cuda_mesh.vertices[triangle.index3].position - cuda_mesh.vertices[triangle.index2].position;
		glm::vec3 vertex3 = cuda_mesh.vertices[triangle.index1].position - cuda_mesh.vertices[triangle.index3].position;
		...
		if (length(alt1) > length(triangle.normal)) 
			triangle.normal = alt1; // higher precision when triangle has sharp angles

		glm::vec3 alt2 = cross(vertex3, vertex1);
		if (length(alt2) > length(triangle.normal)) 
			triangle.normal = alt2;


		triangle.normal = normalize(triangle.normal);

		// precompute dot product between normal and first triangle vertex
		triangle._d = dot(triangle.normal, cuda_mesh.vertices[triangle.index1].position);

		// edge planes
		triangle._e1 = normalize(cross(triangle.normal, vertex1));
		triangle._d1 = dot(triangle._e1, cuda_mesh.vertices[triangle.index1].position);
		triangle._e2 = normalize(cross(triangle.normal, vertex2));
		triangle._d2 = dot(triangle._e2, cuda_mesh.vertices[triangle.index2].position);
		triangle._e3 = normalize(cross(triangle.normal, vertex3));
		triangle._d3 = dot(triangle._e3, cuda_mesh.vertices[triangle.index3].position);
	}
