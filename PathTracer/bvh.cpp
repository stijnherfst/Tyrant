...
BVHNode* Recurse(std::vector<AABB>& work, int depth = 0) {
  ...
  // * SA
  float minimal_cost =
      work.size() * (side1 * side2 + side2 * side3 + side3 * side1);

  float best_split = FLT_MAX;  // Best split along axis, will indicate no split
  ...
  for (int axis = 0; axis < 3; axis++) {  // X, Y, Z
    ...
    for (float test_split = start + step; test_split < stop - step;
      ...
      float total_cost =
          surface_left * count_left + surface_right * count_right;
...
void create_cache_bvh(const Triangle* pFirstTriangle, BVHNode* root,
  ...
  if (!root->IsLeaf()) {  // inner node
    ...
    cache_bvh[currIdxBoxes].u.leaf.count =
        0x80000000 |
        count;  // Highest bit set indicates leaf node, otherwise inner node
...
void CreateCFBVH(CudaMesh cuda_mesh) {
  ...
  if ((idxBoxes != cache_bvh_no - 1) ||
      (idxTriList != triangle_index_list_no)) {
    std::cout << "CreateCFBVH failed\n";
    exit(EXIT_FAILURE);
  }
...
void update_bvh(CudaMesh cuda_mesh, std::string filename) {
  if (!bvh) {
    ...
    if (!fp) {  // Create BVH
      ...
      if (!fp) return;
      if (1 != fwrite(&cache_bvh_no, sizeof(unsigned), 1, fp)) return;
      if (1 != fwrite(&triangle_index_list_no, sizeof(unsigned), 1, fp)) return;
      if (cache_bvh_no !=
          fwrite(cache_bvh, sizeof(CacheFriendlyBVHNode), cache_bvh_no, fp))
        return;
      if (triangle_index_list_no !=
          fwrite(triangle_index_list, sizeof(int), triangle_index_list_no, fp))
        return;

      fclose(fp);
    } else {  // Read existing BVH
      if (1 != fread(&cache_bvh_no, sizeof(unsigned), 1, fp)) return;
      if (1 != fread(&triangle_index_list_no, sizeof(unsigned), 1, fp)) return;

      cache_bvh = new CacheFriendlyBVHNode[cache_bvh_no];
      triangle_index_list = new int[triangle_index_list_no];

      if (cache_bvh_no !=
          fread(cache_bvh, sizeof(CacheFriendlyBVHNode), cache_bvh_no, fp))
        return;
      if (triangle_index_list_no !=
          fread(triangle_index_list, sizeof(int), triangle_index_list_no, fp))
        return;
      fclose(fp);
    }
  }
