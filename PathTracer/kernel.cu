...
struct Ray {
	glm::vec3 orig;
	glm::vec3 dir;
	__device__ Ray(glm::vec3 origin, glm::vec3 direction) : orig(origin), dir(direction) {}
};
...
// Branchless box intersection from https://tavianator.com/fast-branchless-raybounding-box-intersections/
__device__ inline bool RayBoxIntersection(const Ray& ray, int boxIdx, float* cudaBVHlimits) {
	glm::vec3 box_min = { cudaBVHlimits[6 * boxIdx + 0], cudaBVHlimits[6 * boxIdx + 2], cudaBVHlimits[6 * boxIdx + 4] };
	glm::vec3 box_max = { cudaBVHlimits[6 * boxIdx + 1], cudaBVHlimits[6 * boxIdx + 3], cudaBVHlimits[6 * boxIdx + 5] };
...
__device__ bool BVH_IntersectTriangles(int* cudaBVHindexesOrTrilists, const Ray& ray, unsigned avoidSelf, int& triangle_id, float& hit_distance, float* cudaBVHlimits, float* cudaTriangleIntersectionData, int* cudaTriIdxList) {
	triangle_id = -1;
	...
	while (stackIdx) {
		...
		if (!(cudaBVHindexesOrTrilists[4 * boxIdx + 0] & 0x80000000)) {
			...
			for (unsigned i = cudaBVHindexesOrTrilists[4 * boxIdx + 3]; i < cudaBVHindexesOrTrilists[4 * boxIdx + 3] + (cudaBVHindexesOrTrilists[4 * boxIdx + 0] & 0x7fffffff); i++) {
				...
				glm::vec4 normal = glm::vec4(cudaTriangleIntersectionData[20 * idx + 4], cudaTriangleIntersectionData[20 * idx + 5], cudaTriangleIntersectionData[20 * idx + 6], cudaTriangleIntersectionData[20 * idx + 7]);
				...
				glm::vec4 ee1 = glm::vec4(cudaTriangleIntersectionData[20 * idx + 8], cudaTriangleIntersectionData[20 * idx + 9], cudaTriangleIntersectionData[20 * idx + 10], cudaTriangleIntersectionData[20 * idx + 11]);
				float kt1 = dot(ee1, hit) - ee1.w;
				if (kt1<0.0f) continue;

				glm::vec4 ee2 = glm::vec4(cudaTriangleIntersectionData[20 * idx + 12], cudaTriangleIntersectionData[20 * idx + 13], cudaTriangleIntersectionData[20 * idx + 14], cudaTriangleIntersectionData[20 * idx + 15]);
				float kt2 = dot(ee2, hit) - ee2.w;
				if (kt2<0.0f) continue;

				glm::vec4 ee3 = glm::vec4(cudaTriangleIntersectionData[20 * idx + 16], cudaTriangleIntersectionData[20 * idx + 17], cudaTriangleIntersectionData[20 * idx + 18], cudaTriangleIntersectionData[20 * idx + 19]);
...
__device__ inline bool intersect_scene(const Ray& ray, float& t, int& id, int& geometry_type, int* cudaBVHindexesOrTrilists, float* cudaBVHlimits, float* cudaTriangleIntersectionData, int* cudaTriIdxList) {
	...
	// Intersect BVH
	if (BVH_IntersectTriangles(cudaBVHindexesOrTrilists, ray, -1, triangle_id, hit_distance, cudaBVHlimits, cudaTriangleIntersectionData, cudaTriIdxList)) {
		if (hit_distance < t) {
			t = hit_distance;
			geometry_type = 2;
			id = triangle_id;
...
__device__ glm::vec3 radiance(Ray& ray, unsigned int& seed, int* cudaBVHindexesOrTrilists, float* cudaBVHlimits, float* cudaTriangleIntersectionData, int* cudaTriIdxList, Triangle* triangles) {
	...
	for (int bounces = 0; bounces < 4; bounces++) {
		if (!intersect_scene(ray, distance, id, geometry_type, cudaBVHindexesOrTrilists, cudaBVHlimits, cudaTriangleIntersectionData, cudaTriIdxList)) {
		...
		switch (reflection_type) {
			case DIFF: {
				...
				const glm::vec3 u = normalize(glm::cross((abs(normal.x) > .1f ? glm::vec3(0.f, 1.f, 0.f) : glm::vec3(1.f, 0.f, 0.f)), normal));
				const glm::vec3 v = cross(normal, u);
				// Get sample on hemisphere
				const glm::vec3 d = normalize(u * cos(r1) * r2s + v * sin(r1) * r2s + normal * sqrt(1 - r2));
				...
				if (sunLight > 0.0 && !intersect_scene(shadow_ray, shadow_ray_distance, shadow_ray_id, geometry_type, cudaBVHindexesOrTrilists, cudaBVHlimits, cudaTriangleIntersectionData, cudaTriIdxList)) {
...
__global__ void primary_rays(glm::vec3 camera_right, glm::vec3 camera_up, glm::vec3 camera_direction, glm::vec3 O, unsigned int frame, glm::vec4* blit_buffer, int* cudaBVHindexesOrTrilists, float* cudaBVHlimits, float* cudaTriangleIntersectionData, int* cudaTriIdxList, Triangle* triangles) {
	...
	glm::vec3 r = radiance(Ray(O, direction), seed, cudaBVHindexesOrTrilists, cudaBVHlimits, cudaTriangleIntersectionData, cudaTriIdxList, triangles);
...
__global__ void blit_onto_framebuffer(glm::vec4* blit_buffer, unsigned frames) {
	...
	surf2Dwrite<glm::vec4>(glm::pow(color / (color + 1.f), glm::vec4(1.0f/2.2f)), surf, x * sizeof(glm::vec4), y, cudaBoundaryModeZero);
}
...
cudaError_t launch_kernels(cudaArray_const_t array, glm::vec4* blit_buffer, int* cudaBVHindexesOrTrilists, float* cudaBVHlimits, float* cudaTriangleIntersectionData, int* cudaTriIdxList, Triangle* triangles) {
	if (first_time) {
		...
		Sphere sphere_data[5] = {
			{ 16.5,	{ 0, 40, 16.5f },		{ 1, 1, 1 },		DIFF },
	...
	primary_rays <<<blocks, threads>>> (camera_right, camera_up, camera.direction, camera.position, frame, blit_buffer, cudaBVHindexesOrTrilists, cudaBVHlimits, cudaTriangleIntersectionData, cudaTriIdxList, triangles);
